...
class ThreadTestCase(TestCase):
    ...
    def test_add_message_to_thread(self):
        self.thread.users.add(self.user1,self.user2)
        message1 = Messages.objects.create(user=self.user1, content='Muy buenas tardes')
        message2 = Messages.objects.create(user=self.user2, content='Hola') 
    ...
    def test_add_message_from_user_not_in_thread(self):
        ...
        message1 = Messages.objects.create(user=self.user1, content='Muy buenas tardes')
        message2 = Messages.objects.create(user=self.user2, content='Hola')
        message3 = Messages.objects.create(user=self.user3, content='Soy Un espia')
